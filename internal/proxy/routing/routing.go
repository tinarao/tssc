...
func Start(proxyIP string, config *Config) error {
...
func Stop(routingTable int) {
	if err := cleanUpRoutingTable(routingTable); err != nil {
		log.Printf("failed to clean up routing table '%v': %v\n", routingTable, err)
	}
	if err := cleanUpRule(); err != nil {
		log.Printf("failed to clean up IP rule: %v\n", err)
	}
}
...
func setupRoutingTable(routingTable int, tunName, gwSubnet string, tunIP string) error {
	...
	log.Printf("routing traffic from %v to %v through nic %v\n", r.Src, r.Dst, r.LinkIndex)
	...
	log.Printf("routing traffic via gw %v through nic %v...\n", r.Gw, r.LinkIndex)
...
func cleanUpRoutingTable(routingTable int) error {
	...
	if rtDelErr == nil {
		log.Printf("routing table '%v' has been cleaned up\n", routingTable)
	}
	return rtDelErr
}
...
func setupIpRule(svrIp string, routingTable, routingPriority int) error {
	...
	log.Printf("ip rule 'from all not to %v via table %v' created\n", ipRule.Dst, ipRule.Table)
	return nil
}
...
func cleanUpRule() error {
	...
	log.Printf("ip rule of routing table '%v' deleted\n", ipRule.Table)
	ipRule = nil
